...
function loadSettings() {
  try {
    const raw = localStorage.getItem("foodle_settings")
    if (!raw) return
    ...
    const toggle = document.querySelector("#color-blind-theme")
    if (toggle) {
...
function saveSettings() {
  try {
    const on = document.documentElement.classList.contains("colorblind")
    localStorage.setItem("foodle_settings", JSON.stringify({ colorblind: on }))
  } catch (e) {
  }
}
...
function handleMouseClick(e) {
  ...
  } else if (e.target.closest("#color-blind-theme")) {
    const toggle = e.target.closest("#color-blind-theme")
    const on = toggle.toggleAttribute("checked")
    document.documentElement.classList.toggle("colorblind", on)
    document.querySelector("body").classList.toggle("colorblind", on)
    saveSettings()
