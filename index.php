...
// Quick exit for template preview helper
if (isset($_GET['preview_theme'])) {
    require __DIR__ . '/preview_theme.php';
    exit;
}
...
// Master theme preview assets live under /admin/static/... but may still route through index.php
// Serve them the same way as /static/ to ensure Tailwind CSS/JS can be loaded during preview.
if (strpos($path, '/admin/static/') === 0) {
    $static_file = ltrim($path, '/');
    if (file_exists($static_file)) {
        $ext = pathinfo($static_file, PATHINFO_EXTENSION);
        $mime_types = [
...
if (file_exists('vendor/autoload.php')) {
    ...
    $templateEngine = new SimpleTemplate("tpl/{$domain}");
    $templateEngine->addFunction('__', '__');
    $templateEngine->addFunction('get_language_url', 'get_language_url');
    $templateEngine->addFunction('get_page_url', 'get_page_url');
    $templateEngine->addFunction('get_static_url', 'get_static_url');
...
try {
    ...
    if ($use_twig) {
        ...
        echo $templateEngine->render($template, $template_vars);
    }
} catch (Exception $e) {
