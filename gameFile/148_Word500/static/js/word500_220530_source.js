...
const keys = ['US', 'Q', 'W', 'E', 'R', 'T', 'Y', 'U', 'I', 'O', 'P', 'sp', 'Hint', 'A', 'S', 'D', 'F', 'G', 'H', 'J', 'K', 'L', 'sp', 'Back', 'Z', 'X', 'C', 'V', 'B', 'N', 'M', 'Spec', 'Enter', 'sp',];
...
let mode = window.location.search?.includes('mode=practice') ? 'E' : 'A'; // A=Daily, B=Timed, C=Speedrun, D=Custom, E = practice
...
window.onload = function () {
	...
	if (window.location.search?.includes('mode=practice')) {
		document.title = 'Practice Word500'
	} else if (window.location.search?.includes('mode=daily')) {
		document.title = 'Daily Word500'
	}

	if (window.location.search?.includes('mode=practice')) {
		document.getElementById('btnMode').src = 'images/modeE.png';
	} else {
		document.getElementById('btnMode').src = 'images/modeA.png';
	}
};

function ini() {

	const keyboardContainer = document.getElementById('divkeyboard');
	...
	// 	buttonElement.classList.add('clickable');
	// 	buttonElement.classList.add('mystyle');
	// 	buttonElement.addEventListener('click', () => handleClick(key));
	// 	divkeyboard.append(buttonElement);
	// 	if (key === 'sp') {
	// 		buttonElement.classList.remove('clickable');
	// 		buttonElement.classList.remove('unusedKey');
	// 		buttonElement.classList.add('mystyle');
	...
	if (lang == null) {
		...
		word = pu(up('word')) === 'NONE' ? word : 'error'; //read custom word from URL
	}
	rg(); //ResetGame
	...
	if (mode !== 'D') {
		...
		if (lastDate == null) {
			...
			ll1l1('newplayer');
			// lastWord = todayWords[0]
			// lastWordIndex = words.findIndex(val => val === lastWord)
			// console.log(lastWordIndex) 
			// localStorage.setItem('lastword', lastWordIndex)

			// var testa = words[localStorage.getItem('lastword')]
			// var testb = llll1(testa)
			// console.log('newplayer', testa, testb)
			if (mode !== 'E') {
				word = llll1(localStorage.getItem(lang + level + 'word')); //Decrypt
			}
			// word = testb
			// console.log(word, localStorage.getItem(lang + level + 'word'))
		} else if (mode === 'E') {
			startPractice();
			// do nothing
			...
			// var testa = words[localStorage.getItem('lastword')]
			// var testb = llll1(testa)
			// console.log('newday', testa, testb)
			localStorage.setItem('word500date', today);
			word = llll1(localStorage.getItem(lang + level + 'word')); //Decrypt
			// console.log(word, localStorage.getItem(lang + level + 'word'))
			// word = testb;
	...
	document.getElementById('modeA').addEventListener('click', (e) => mode_Click('A', e));
	document.getElementById('modeB').addEventListener('click', (e) => mode_Click('B', e));
	document.getElementById('modeC').addEventListener('click', (e) => mode_Click('C', e));
	document.getElementById('modeE').addEventListener('click', (e) => mode_Click('E', e));
	...
	document.getElementById('word500logo').addEventListener('click', () => dcl());
...
function pu(uwl){ //ProcessURL word
	if (uwl.length == 10) {
		...
		console.log('Error: invalid custom URL');
	}
	// No valid word grabbed from URL
	mode = window.location.search?.includes('mode=practice') ? 'E' : 'A';
	lang = localStorage.getItem('word500lang');
	if (lang == null) {
		lang = 'en';
	}
...
const dcl = () => { //DoubleClickLogo
	...
	if (d - clickTime < 500) {
		...
		//Force color letters red
		for (let i = 0; i < row; i++) {
			if (document.getElementById(i.toString() + '-' + (width + 2).toString()).innerText == '5') {
				cred(i,false);
			}
		}
	}
...
const rtr = () => { //restoreSession
	word = llll1(localStorage.getItem(lang + level + 'word')); //Decrypt
	// console.log(words)
	// var test1 = localStorage.getItem('lastword');
	// var test2 = words[test1]
	// var test3 = llll1(test2);
	// console.log('same day', word, localStorage.getItem(lang + level + 'word'))
	// word = test3;
...
const ll1l1 = (gametype) => { //GetWords from list


		let enOffset = Number(dayOffset)
		for (let i = 0; i < 24; i++) {
			if (i < 3) {
				todayWords.push(words[enOffset])
				enOffset++
			} else {
				todayWords.push('AAAAA')
			}		
		}
		// console.log(todayWords)
		let newWords = todayWords;
		let i = 0;
		langs.forEach(lng => {
			levels.forEach(lvl => {
...
const cB = (r,c) => { //ClickBoard
	if (r < row && !gameOver && c < width) {
		// Change tile color
...
const mode_Click = (char, event) => {
	...
	if (mode == 'A') {
		//rtr(); //restoreSession
		//document.getElementById('title').innerText = 'Word500'
		//history.pushState(state, '', '/game?mode=daily');
	...
	if (mode == 'E') {
		// startPractice();
		//history.pushState(state, '', '/game?mode=practice');
...
const wru = (result, rowNumber) => { //writeResult
	if (result > 100) {
		...
		result='005';
		//color red and delay if mode = ADE
		cred(rowNumber, (mode == 'A' || mode == 'D' || mode == 'E'));
	}
...
const cred = (rn, delay) => {//Color red
		let d = 0;
		if ((mode == 'A' || mode == 'D' || mode == 'E') && delay) {
			d = 500;
		}
		for (let i = 0; i < width; i++) {
			let b = rn;
		...
		for (let i = 0; i < width; i++) {
			for (let j = 0; j < row; j++) {
				if (guesses[rn].includes(document.getElementById(j.toString() + '-' + i.toString()).innerText)) {
					setTimeout(() => {
...
const ll11l = () => { //ProcessEnter testx
	if (col < 5) return;
	...
	if (!gameOver && row == height && mode == 'A') {
		...
		sm('Game over! The word was ' + word);
		ab();
	} else if (!gameOver && row == height && (mode == 'D' || mode == 'E')) {
		gameOver = true;
		document.getElementsByClassName('restartButton')[0].style.display = 'flex';
		sm('Game over! The word was ' + word);
		ab();
	}
}
...
const link_Click = () => {
	...
	if (letter == '') {
		...
		if (ll111(word0) && !word0.includes('_')) {
			//Valid custom word
			document.getElementById('customwordlabel').innerText = 'The custom secret word is ' + word0 + '\n\nCopy the below URL and send it to your friend(s)!';
			document.getElementById('customword').innerText = 'https://word500.com?lang=' + c2u(word0); //Convert to URL
		} else {
			//Invalid custom word
			document.getElementById('customwordlabel').innerText = 'No valid word has been written';
		}
...
function cc(m) { //Copy to Clipboard
	...
	let result;
	word = llll1(localStorage.getItem(lang + level + 'word')); //Decrypt
	// console.log(localStorage.getItem(lang + level + 'word'))
	hintsUsed = parseInt(localStorage.getItem(lang + level + 'hints'));
