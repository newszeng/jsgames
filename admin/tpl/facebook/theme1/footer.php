    ...
    function downloadVideo() {
        ...
        if (!url) {
            showAlert('<?php echo __('error.empty_url', 'Please enter a Facebook video URL'); ?>', 'warning');
            return;
        }
        
        // Validate Facebook URL or search term
        const isUrl = url.includes('http');
        if (isUrl && !url.includes('facebook.com') && !url.includes('fb.watch') && !url.includes('fb.com')) {
            showAlert('<?php echo __('error.invalid_url', 'Please enter a valid Facebook URL'); ?>', 'danger');
            return;
        }
