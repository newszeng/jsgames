    <!-- InstaDown Footer Structure -->
    <footer style="background-color: #161823; color: white; padding: 60px 0 30px;">
        <div class="container">
            <div class="text-center">
                <!-- Static Pages Links -->
                <div style="margin-bottom: 20px;">
                    <a href="<?php echo get_page_url('/terms'); ?>" style="color: #E4405F; margin: 0 10px;">Terms of Service</a>
                    <a href="<?php echo get_page_url('/privacy'); ?>" style="color: #E4405F; margin: 0 10px;">Privacy Policy</a>
                    <a href="<?php echo get_page_url('/dmca'); ?>" style="color: #E4405F; margin: 0 10px;">DMCA</a>
                    <a href="<?php echo get_page_url('/contact'); ?>" style="color: #E4405F; margin: 0 10px;">Contact Us</a>
                </div>
                
                <p>© <?php echo date('Y'); ?> <a href="<?php echo get_page_url('/'); ?>" style="color: #E4405F;"><?php echo __('site_name', 'InstaDown'); ?></a>. All rights reserved. </p>
                
                <?php
                // Display footer backlinks like InstaDown
                ...
                if (file_exists($backlinks_file)) {
                    ...
                    if (!empty($footer_links)) {
                        ...
                        foreach ($footer_links as $link) {
                            echo '<a href="' . htmlspecialchars($link['url']) . '" target="' . ($link['target'] ?? '_blank') . '" rel="' . ($link['rel'] ?? 'nofollow') . '" style="color: #E4405F; margin: 0 10px;">' . htmlspecialchars($link['title']) . '</a>';
                        }
                        echo '</div>';
                    }
                }
    ...
    <!-- Bootstrap JS (Compatible with InstaDown structure) -->
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.6.0/dist/js/bootstrap.bundle.min.js"></script>
    
    <!-- InstaDown Original JavaScript Functions -->
    <script>
        // InstaDown-style paste functionality
        $(document).ready(function() {
            ...
            $('.downloadform').submit(function(e) {
                ...
                loadingSpinner.show();
                resultsContainer.html('<div class="text-center py-4"><img src="/loading.gif" alt="Loading..." style="width: 50px; height: 50px;"><p class="mt-3">' + '<?php echo __('processing_text', 'Processing your Instagram video...'); ?>' + '</p></div>');
            ...
            // FAQ accordion functionality (InstaDown style)
    ...
    function downloadVideo() {
        ...
        if (!url) {
            showAlert('<?php echo __('error.empty_url', 'Please enter an Instagram video URL'); ?>', 'warning');
            return;
        }
        
        // Validate Instagram URL or search term
        const isUrl = url.includes('http');
        if (isUrl && !url.includes('instagram.com')) {
            showAlert('<?php echo __('error.invalid_url', 'Please enter a valid Instagram URL'); ?>', 'danger');
            return;
        }
