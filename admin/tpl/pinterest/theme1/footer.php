    <!-- Pinterest Downloader Footer Structure -->
    <footer style="background-color: #111118; color: white; padding: 60px 0 30px;">
        <div class="container">
            <div class="text-center">
                <!-- Static Pages Links -->
                <div style="margin-bottom: 20px;">
                    <a href="<?php echo get_page_url('/terms'); ?>" style="color: #BD081C; margin: 0 10px;">Terms of Service</a>
                    <a href="<?php echo get_page_url('/privacy'); ?>" style="color: #BD081C; margin: 0 10px;">Privacy Policy</a>
                    <a href="<?php echo get_page_url('/dmca'); ?>" style="color: #BD081C; margin: 0 10px;">DMCA</a>
                    <a href="<?php echo get_page_url('/contact'); ?>" style="color: #BD081C; margin: 0 10px;">Contact Us</a>
                </div>
                
                <p>© <?php echo date('Y'); ?> <a href="<?php echo get_page_url('/'); ?>" style="color: #BD081C;"><?php echo __('site_name', 'Pinterest Downloader'); ?></a>. All rights reserved. </p>
                ...
                if (file_exists($backlinks_file)) {
                    ...
                    if (!empty($footer_links)) {
                        ...
                        foreach ($footer_links as $link) {
                            echo '<a href="' . htmlspecialchars($link['url']) . '" target="' . ($link['target'] ?? '_blank') . '" rel="' . ($link['rel'] ?? 'nofollow') . '" style="color: #BD081C; margin: 0 10px;">' . htmlspecialchars($link['title']) . '</a>'; 
                        }
                        echo '</div>';
                    }
                }
    ...
    <!-- Pinterest Downloader JavaScript Functions -->
    <script>
        // Pinterest-style paste functionality
        $(document).ready(function() {
            ...
            $('.downloadform').submit(function(e) {
                ...
                if (!url) {
                    alert('Please enter a Pinterest video or Pin URL');
                    return;
                }
                ...
                loadingSpinner.show();
                resultsContainer.html('<div class="text-center py-4"><img src="/loading.gif" alt="Loading..." style="width: 50px; height: 50px;"><p class="mt-3">' + '<?php echo __('processing_text', 'Processing your Pinterest video...'); ?>' + '</p></div>');
    ...
    function downloadVideo() {
        ...
        if (!url) {
            showAlert('<?php echo __('error.empty_url', 'Please enter a Pinterest video or Pin URL'); ?>', 'warning');
            return;
        }
        
        // Validate Pinterest URL or search term
        const isUrl = url.includes('http');
        if (isUrl && !url.includes('pinterest.com') && !url.includes('pin.it')) {
            showAlert('<?php echo __('error.invalid_url', 'Please enter a valid Pinterest URL'); ?>', 'danger');
            return;
        }
