 ...
 */
require_once __DIR__ . '/../admin/lib/translate_helper_chatgpt.php';

// 获取命令行参数
if ($argc < 4) {
    echo "使用方法: php page_translate_chatgpt.php <site> <page> <lang>\n";
    echo "示例: php page_translate_chatgpt.php ytdownloader.io homepage zh\n";
    echo "     php page_translate_chatgpt.php ytdownloader.io homepage all\n";
    exit(1);
}
...
// 源文件路径
$source_file = __DIR__ . "/../i18n/{$site}/en.json";
...
if ($page === 'homepage') {
    $page_content = [
        ...
        'static_pages' => $source_array['static_pages'] ?? []
    ];
} elseif ($page === 'tagpage') {
    $page_content = [
...
echo "===================================\n";
echo "使用 ChatGPT 3.5 翻译\n";
echo "===================================\n";
echo "翻译 {$page} 页面到 " . count($target_languages) . " 种语言\n";
echo "===================================\n\n";
...
foreach ($target_languages as $targetLang) {
    ...
    $target_file = __DIR__ . "/../i18n/{$site}/{$targetLang}.json";
    ...
    try {
        // 使用一次性翻译整个JSON
        $translated_content = translateJsonWithChatGPT($page_content, $targetLang, 'English');
        ...
        if ($translated_content) {
            ...
            $lang_time = round(microtime(true) - $lang_start_time, 2);
            echo "✅ 成功保存到: {$target_file}\n";
            echo "   单语言耗时: {$lang_time} 秒\n";
        } else {
            echo "❌ 翻译失败，尝试分批翻译...\n";
            // 降级到分批翻译
            $translated_content = translateJsonContentWithChatGPT($page_content, 'en', $targetLang);
