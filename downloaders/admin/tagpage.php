<?php
// 这个文件被 website_pages.php 包含，$domain 变量已经可用
require_once __DIR__ . '/lib/translate_helper_chatgpt.php';

$pages_dir = "../pages/{$domain}";
$current_lang = $_GET['lang'] ?? 'en';
$error = '';
$success = '';

// 检查是否有翻译成功的提示
if (isset($_GET['translated'])) {
    $success = '翻译完成';
}

// 获取所有标签页面
$tag_pages = [];
if (is_dir($pages_dir)) {
    $dirs = scandir($pages_dir);
    foreach ($dirs as $dir) {
        if ($dir != '.' && $dir != '..' && is_dir("{$pages_dir}/{$dir}")) {
            $tag_pages[] = $dir;
        }
    }
}

// 从模板文件提取默认内容的函数
function extractDefaultContent($domain) {
    $template_dir = "../tpl/{$domain}";
    $i18n_file = "../i18n/{$domain}/en.json";
    $default_content = [];
    
    // 读取i18n数据作为基础
    $i18n_data = [];
    if (file_exists($i18n_file)) {
        $i18n_data = json_decode(file_get_contents($i18n_file), true) ?: [];
    }
    
    // 动态解析模板文件
    $extracted_keys = [];
    
    // 解析模板文件中的__()调用
    $template_files = ['header.php', 'page.php', 'footer.php'];
    foreach ($template_files as $file) {
        $file_path = "{$template_dir}/{$file}";
        if (file_exists($file_path)) {
            $content = file_get_contents($file_path);
            
            // 匹配所有 __('key', 'default') 调用
            preg_match_all("/\b__\(\s*['\"]([^'\"]+)['\"]\s*(?:,\s*['\"]([^'\"]*)['\"])?\s*\)/", $content, $matches, PREG_SET_ORDER);
            
            foreach ($matches as $match) {
                $key = $match[1];
                $default_value = $match[2] ?? '';
                
                // 解析嵌套键如 'page.title'
                $keys = explode('.', $key);
                $extracted_keys[$key] = [
                    'keys' => $keys,
                    'default' => $default_value,
                    'file' => $file
                ];
            }
            
            // 匹配 str_replace('{site_name}', __('site_name'), __('key', 'default')) 调用
            preg_match_all("/str_replace\([^,]+,\s*[^,]+,\s*\b__\(\s*['\"]([^'\"]+)['\"]\s*(?:,\s*['\"]([^'\"]*)['\"])?\s*\)\)/", $content, $str_matches, PREG_SET_ORDER);
            
            foreach ($str_matches as $match) {
                $key = $match[1];
                $default_value = $match[2] ?? '';
                
                $keys = explode('.', $key);
                $extracted_keys[$key] = [
                    'keys' => $keys,
                    'default' => $default_value,
                    'file' => $file,
                    'has_site_name_placeholder' => true
                ];
            }
            
            // 匹配placeholder属性中的__()调用
            preg_match_all('/placeholder\s*=\s*["\'](?:[^"\']*\?php\s+echo\s+)?(?:[^"\']*__\(\s*[\'"]([^"\']+)[\'"]\s*(?:,\s*[\'"]([^"\']*)[\'"]\s*)?\s*\)[^"\']*)?["\']/', $content, $placeholder_matches, PREG_SET_ORDER);
            
            foreach ($placeholder_matches as $match) {
                if (isset($match[1])) {
                    $key = $match[1];
                    $default_value = $match[2] ?? '';
                    
                    $keys = explode('.', $key);
                    $extracted_keys[$key] = [
                        'keys' => $keys,
                        'default' => $default_value,
                        'file' => $file,
                        'type' => 'placeholder'
                    ];
                }
            }
            
            // 匹配按钮文本中的__()调用
            preg_match_all('/<button[^>]*>(?:[^<]*\?php\s+echo\s+)?(?:[^<]*__\(\s*[\'"]([^"\']+)[\'"]\s*(?:,\s*[\'"]([^"\']*)[\'"]\s*)?\s*\)[^<]*)?<\/button>/', $content, $button_matches, PREG_SET_ORDER);
            
            foreach ($button_matches as $match) {
                if (isset($match[1])) {
                    $key = $match[1];
                    $default_value = $match[2] ?? '';
                    
                    $keys = explode('.', $key);
                    $extracted_keys[$key] = [
                        'keys' => $keys,
                        'default' => $default_value,
                        'file' => $file,
                        'type' => 'button'
                    ];
                }
            }
            
            // 匹配 $page_data['key'] ?? __('fallback') 模式
            preg_match_all('/\$page_data\[[\'"]([^\'"]+)[\'"]\]\[[\'"]([^\'"]+)[\'"]\]\s*\?\?\s*__\(\s*[\'"]([^"\']+)[\'"]\s*(?:,\s*[\'"]([^"\']*)[\'"]\s*)?\s*\)/', $content, $page_data_matches, PREG_SET_ORDER);
            
            foreach ($page_data_matches as $match) {
                $section = $match[1];
                $key = $match[2];
                $fallback_key = $match[3];
                $default_value = $match[4] ?? '';
                
                $full_key = "{$section}.{$key}";
                $keys = [$section, $key];
                
                $extracted_keys[$full_key] = [
                    'keys' => $keys,
                    'default' => $default_value,
                    'file' => $file,
                    'fallback_key' => $fallback_key
                ];
            }
            
            // 匹配数组结构 $page_data['features']['unlimited']['title']
            preg_match_all('/\$page_data\[[\'"]([^\'"]+)[\'"]\]\[[\'"]([^\'"]+)[\'"]\]\[[\'"]([^\'"]+)[\'"]\]/', $content, $nested_matches, PREG_SET_ORDER);
            
            foreach ($nested_matches as $match) {
                $section = $match[1];
                $subsection = $match[2];  
                $key = $match[3];
                
                $full_key = "{$section}.{$subsection}.{$key}";
                $keys = [$section, $subsection, $key];
                
                if (!isset($extracted_keys[$full_key])) {
                    $extracted_keys[$full_key] = [
                        'keys' => $keys,
                        'default' => "Default {$full_key}",
                        'file' => $file,
                        'type' => 'nested_array'
                    ];
                }
            }
            
            // 解析列表项中的内容 (如指令步骤、优势列表)
            if (preg_match_all('/<ol[^>]*>(.*?)<\/ol>/s', $content, $ol_matches)) {
                foreach ($ol_matches[1] as $ol_content) {
                    preg_match_all('/<li[^>]*>(?:[^<]*\?php\s+echo\s+)?(?:[^<]*__\(\s*[\'"]([^"\']+)[\'"]\s*(?:,\s*[\'"]([^"\']*)[\'"]\s*)?\s*\)[^<]*)?<\/li>/', $ol_content, $li_matches, PREG_SET_ORDER);
                    
                    foreach ($li_matches as $match) {
                        if (isset($match[1])) {
                            $key = $match[1];
                            $default_value = $match[2] ?? '';
                            
                            $keys = explode('.', $key);
                            $extracted_keys[$key] = [
                                'keys' => $keys,
                                'default' => $default_value,
                                'file' => $file,
                                'type' => 'list_item'
                            ];
                        }
                    }
                }
            }
            
            if (preg_match_all('/<ul[^>]*>(.*?)<\/ul>/s', $content, $ul_matches)) {
                foreach ($ul_matches[1] as $ul_content) {
                    preg_match_all('/<li[^>]*>(?:[^<]*\?php\s+echo\s+)?(?:[^<]*__\(\s*[\'"]([^"\']+)[\'"]\s*(?:,\s*[\'"]([^"\']*)[\'"]\s*)?\s*\)[^<]*)?<\/li>/', $ul_content, $li_matches, PREG_SET_ORDER);
                    
                    foreach ($li_matches as $match) {
                        if (isset($match[1])) {
                            $key = $match[1];
                            $default_value = $match[2] ?? '';
                            
                            $keys = explode('.', $key);
                            $extracted_keys[$key] = [
                                'keys' => $keys,
                                'default' => $default_value,
                                'file' => $file,
                                'type' => 'list_item'
                            ];
                        }
                    }
                }
            }
        }
    }
    
    // 构建默认内容结构
    $default_content = [
        'site_name' => $i18n_data['site_name'] ?? ucfirst(explode('.', $domain)[0]),
        'created_at' => date('Y-m-d H:i:s')
    ];
    
    // 根据解析出的键构建嵌套结构
    foreach ($extracted_keys as $key => $info) {
        $keys = $info['keys'];
        $value = '';
        
        // 优先使用i18n中的值，否则使用默认值
        $nested_value = $i18n_data;
        foreach ($keys as $k) {
            if (isset($nested_value[$k])) {
                $nested_value = $nested_value[$k];
            } else {
                $nested_value = null;
                break;
            }
        }
        
        $value = $nested_value ?? $info['default'] ?? "Default {$key} content";
        
        // 设置嵌套值
        $current = &$default_content;
        for ($i = 0; $i < count($keys) - 1; $i++) {
            if (!isset($current[$keys[$i]])) {
                $current[$keys[$i]] = [];
            }
            $current = &$current[$keys[$i]];
        }
        $current[$keys[count($keys) - 1]] = $value;
    }
    
    // 确保基本结构存在
    if (!isset($default_content['page'])) {
        $default_content['page'] = [];
    }
    
    // 设置一些通用的默认值
    $site_name = $default_content['site_name'];
    
    if (!isset($default_content['page']['title'])) {
        $default_content['page']['title'] = $site_name;
    }
    
    if (!isset($default_content['page']['meta_description'])) {
        $default_content['page']['meta_description'] = "Download videos from {$site_name} - Fast, Free, and Easy to Use";
    }
    
    // 添加自定义内容区域
    if (!isset($default_content['custom_content'])) {
        $default_content['custom_content'] = [
            'title' => 'Additional Information',
            'content' => '<div><p>Add your custom content here. This section will appear in the page template.</p></div>'
        ];
    }
    
    return $default_content;
}

// 处理一键翻译所有语言请求
if ((isset($_GET['translate_all']) || isset($_GET['translate_remaining'])) && isset($_GET['slug'])) {
    $source_lang = isset($_GET['translate_all']) ? $_GET['translate_all'] : $_GET['translate_remaining'];
    $translate_mode = isset($_GET['translate_all']) ? 'all' : 'remaining';
    $slug = $_GET['slug'];
    
    // 读取源语言文件
    $source_file = "{$pages_dir}/{$slug}/{$source_lang}.json";
    if (!file_exists($source_file)) {
        $error = "源语言文件未找到";
    } else {
        $source_content = file_get_contents($source_file);
        $source_array = json_decode($source_content, true);
        
        if (!$source_array) {
            $error = "源语言文件格式错误";
        } else {
            // 获取所有支持的语言
            $all_languages = getSupportedLanguages();
            $target_languages = [];
            
            foreach ($all_languages as $code => $name) {
                if ($code !== $source_lang && $code !== 'en') { // 跳过源语言和英语
                    if ($translate_mode === 'all') {
                        // 翻译所有语言
                        $target_languages[] = $code;
                    } else {
                        // 只翻译剩余（不存在翻译文件的）语言
                        $target_file = "{$pages_dir}/{$slug}/{$code}.json";
                        if (!file_exists($target_file)) {
                            $target_languages[] = $code;
                        }
                    }
                }
            }
            
            // 检查是否有需要翻译的语言
            if (empty($target_languages)) {
                echo '<!DOCTYPE html><html><head><title>翻译完成</title></head><body>';
                echo '<div style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; padding: 30px; border: 2px solid #28a745; border-radius: 10px; text-align: center;">';
                echo '<h3><i class="fas fa-check-circle" style="color: #28a745;"></i> 所有语言都已翻译完成！</h3>';
                echo '<p>没有需要翻译的剩余语言。</p>';
                echo '<button onclick="window.location.href=\'website_pages.php?domain=' . $domain . '&act=tagpage&edit=' . $slug . '&lang=' . $source_lang . '\'" class="btn btn-primary">返回</button>';
                echo '</div></body></html>';
                exit();
            }
            
            // 开始AJAX翻译流程
            $mode_title = $translate_mode === 'all' ? '一键翻译全部' : '翻译剩余语言';
            echo '<!DOCTYPE html><html><head><title>翻译进行中</title><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css"></head><body>';
            echo '<div id="translation-progress" style="position: fixed; top: 20%; left: 50%; transform: translate(-50%, -20%); background: white; padding: 30px; border: 2px solid #007cba; border-radius: 10px; z-index: 9999; box-shadow: 0 0 20px rgba(0,0,0,0.3); min-width: 400px; max-height: 60vh; overflow-y: auto;">';
            echo '<h3><i class="fas fa-language"></i> ' . $mode_title . '进行中...</h3>';
            echo '<div id="progress-info">准备翻译到 ' . count($target_languages) . ' 种语言</div>';
            echo '<div style="margin: 20px 0;"><div id="progress-bar" style="width: 100%; background: #f0f0f0; border-radius: 10px; overflow: hidden;"><div id="progress-fill" style="width: 0%; height: 30px; background: linear-gradient(45deg, #007cba, #00a0d2); transition: width 0.3s;"></div></div></div>';
            echo '<div id="current-lang">等待开始...</div>';
            echo '<div id="remaining-count">剩余: ' . count($target_languages) . ' 种语言</div>';
            echo '</div>';
            
            echo '<script>
            let languages = ' . json_encode($target_languages) . ';
            let currentIndex = 0;
            let totalLanguages = languages.length;
            let sourceLang = "' . $source_lang . '";
            let slug = "' . $slug . '";
            
            function translateNext() {
                if (currentIndex >= languages.length) {
                    document.getElementById("current-lang").innerHTML = "✅ 翻译完成！";
                    document.getElementById("remaining-count").innerHTML = "所有语言翻译完成";
                    setTimeout(function() {
                        window.location.href = "website_pages.php?domain=' . $domain . '&act=tagpage&edit=" + slug + "&lang=" + sourceLang + "&translated_all=1";
                    }, 2000);
                    return;
                }
                
                let targetLang = languages[currentIndex];
                let progress = Math.round(((currentIndex) / totalLanguages) * 100);
                let remaining = totalLanguages - currentIndex;
                
                document.getElementById("progress-fill").style.width = progress + "%";
                document.getElementById("current-lang").innerHTML = "🔄 正在翻译: " + targetLang.toUpperCase();
                document.getElementById("remaining-count").innerHTML = "剩余: " + remaining + " 种语言";
                
                // 发送AJAX请求
                fetch("website_pages.php?domain=' . $domain . '&act=tagpage&ajax_translate=1&source_lang=" + sourceLang + "&target_lang=" + targetLang + "&slug=" + slug)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        document.getElementById("current-lang").innerHTML = "✅ 完成: " + targetLang.toUpperCase();
                    } else {
                        document.getElementById("current-lang").innerHTML = "❌ 失败: " + targetLang.toUpperCase() + " - " + data.error;
                    }
                    
                    currentIndex++;
                    setTimeout(translateNext, 1000); // 等待1秒再翻译下一个
                })
                .catch(error => {
                    document.getElementById("current-lang").innerHTML = "❌ 错误: " + targetLang.toUpperCase() + " - " + error;
                    currentIndex++;
                    setTimeout(translateNext, 1000);
                });
            }
            
            // 开始翻译
            setTimeout(translateNext, 1000);
            </script></body></html>';
            
            exit();
        }
    }
}

// 处理AJAX翻译请求
if (isset($_GET['ajax_translate']) && isset($_GET['slug'])) {
    $source_lang = $_GET['source_lang'];
    $target_lang = $_GET['target_lang'];
    $slug = $_GET['slug'];
    
    header('Content-Type: application/json');
    
    $source_file = "{$pages_dir}/{$slug}/{$source_lang}.json";
    if (!file_exists($source_file)) {
        echo json_encode(['success' => false, 'error' => '源文件不存在']);
        exit();
    }
    
    $source_content = file_get_contents($source_file);
    $source_array = json_decode($source_content, true);
    
    if (!$source_array) {
        echo json_encode(['success' => false, 'error' => '源文件格式错误']);
        exit();
    }
    
    try {
        // 执行翻译 - 使用ChatGPT一次性翻译
        $translated_array = translateJsonWithChatGPT($source_array, $target_lang, 'English');
        
        // 如果一次性翻译失败，降级到分批翻译
        if (!$translated_array) {
            $translated_array = translateJsonContentWithChatGPT($source_array, $source_lang, $target_lang);
        }
        
        if ($translated_array) {
            // 确保目录存在
            $target_dir = dirname("{$pages_dir}/{$slug}/{$target_lang}.json");
            if (!is_dir($target_dir)) {
                mkdir($target_dir, 0777, true);
            }
            
            // 保存翻译后的内容
            $target_file = "{$pages_dir}/{$slug}/{$target_lang}.json";
            file_put_contents($target_file, json_encode($translated_array, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
            
            echo json_encode(['success' => true]);
        } else {
            echo json_encode(['success' => false, 'error' => '翻译返回空结果']);
        }
    } catch (Exception $e) {
        echo json_encode(['success' => false, 'error' => $e->getMessage()]);
    }
    
    exit();
}

// 处理更新路由请求
if (isset($_GET['update_router'])) {
    header('Content-Type: application/json');
    
    try {
        // 获取当前所有语言
        $available_languages = [];
        $i18n_dir = "../i18n/{$domain}";
        if (is_dir($i18n_dir)) {
            $files = scandir($i18n_dir);
            foreach ($files as $file) {
                if (preg_match('/^([a-z]{2})\.json$/', $file, $matches)) {
                    $available_languages[] = $matches[1];
                }
            }
        }
        
        // 读取router.json文件
        $router_file = "../router/{$domain}/router.json";
        if (!file_exists($router_file)) {
            echo json_encode(['success' => false, 'error' => 'router.json文件不存在']);
            exit();
        }
        
        $router_content = file_get_contents($router_file);
        $router_data = json_decode($router_content, true);
        
        if (!$router_data) {
            echo json_encode(['success' => false, 'error' => 'router.json格式错误']);
            exit();
        }
        
        // 更新languages数组
        $router_data['languages'] = $available_languages;
        
        // 保存更新后的router.json
        $updated_content = json_encode($router_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
        file_put_contents($router_file, $updated_content);
        
        echo json_encode([
            'success' => true, 
            'languages' => $available_languages,
            'message' => '路由文件已更新，支持 ' . count($available_languages) . ' 种语言'
        ]);
        
    } catch (Exception $e) {
        echo json_encode(['success' => false, 'error' => $e->getMessage()]);
    }
    
    exit();
}

// 处理翻译请求
if (isset($_GET['translate']) && isset($_GET['target_lang']) && isset($_GET['slug'])) {
    $source_lang = $_GET['translate'];
    $target_lang = $_GET['target_lang'];
    $slug = $_GET['slug'];
    
    // 读取源语言文件
    $source_file = "{$pages_dir}/{$slug}/{$source_lang}.json";
    if (file_exists($source_file)) {
        $source_content = file_get_contents($source_file);
        $source_array = json_decode($source_content, true);
        
        if ($source_array) {
            try {
                // 执行翻译 - 使用ChatGPT一次性翻译
                $translated_array = translateJsonWithChatGPT($source_array, $target_lang, 'English');
                
                // 如果一次性翻译失败，降级到分批翻译
                if (!$translated_array) {
                    $translated_array = translateJsonContentWithChatGPT($source_array, $source_lang, $target_lang);
                }
                
                // 保存翻译后的内容
                $target_file = "{$pages_dir}/{$slug}/{$target_lang}.json";
                file_put_contents($target_file, json_encode($translated_array, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
                
                $success = "成功将 " . strtoupper($source_lang) . " 翻译为 " . strtoupper($target_lang);
                
                // 使用JavaScript重定向
                echo "<script>window.location.href='website_pages.php?domain={$domain}&act=tagpage&edit={$slug}&lang={$target_lang}&translated=1';</script>";
                exit();
            } catch (Exception $e) {
                $error = "翻译失败: " . $e->getMessage();
            }
        } else {
            $error = "源语言文件格式错误";
        }
    } else {
        $error = "源语言文件不存在";
    }
}

// 处理创建新标签页
if (isset($_POST['create_page'])) {
    $slug = preg_replace('/[^a-z0-9\-]/', '', strtolower(str_replace(' ', '-', $_POST['slug'])));
    $keyword = trim($_POST['keyword'] ?? '');
    $json_content = $_POST['json_content'] ?? '';
    
    if (empty($slug)) {
        $error = 'URL地址格式无效';
    } elseif (empty($keyword)) {
        $error = '关键词不能为空';
    } elseif (in_array($slug, $tag_pages)) {
        $error = '该URL地址的页面已存在';
    } else {
        // 验证 JSON 格式
        $page_data = json_decode($json_content, true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $error = 'JSON格式错误: ' . json_last_error_msg();
        } else {
            // 处理关键词 - 每个单词首字母大写
            $formatted_keyword = ucwords(strtolower($keyword));
            
            // 替换 page.title
            if (isset($page_data['page']['title'])) {
                $page_data['page']['title'] = $formatted_keyword;
            }
            
            // 创建页面目录
            $page_dir = "{$pages_dir}/{$slug}";
            @mkdir($page_dir, 0777, true);
            
            // 保存更新后的JSON文件
            $updated_json = json_encode($page_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
            file_put_contents("{$page_dir}/{$current_lang}.json", $updated_json);
            $success = '标签页面创建成功';
            $tag_pages[] = $slug;
        }
    }
}

// 处理编辑页面
if (isset($_POST['edit_page'])) {
    $slug = $_POST['slug'] ?? '';
    $json_content = $_POST['json_content'] ?? '';
    $page_dir = "{$pages_dir}/{$slug}";
    
    if (is_dir($page_dir)) {
        // 验证 JSON 格式
        $page_data = json_decode($json_content, true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $error = 'JSON格式错误: ' . json_last_error_msg();
        } else {
            // 保持创建时间
            $existing_file = "{$page_dir}/{$current_lang}.json";
            if (file_exists($existing_file)) {
                $existing_data = json_decode(file_get_contents($existing_file), true);
                $page_data['created_at'] = $existing_data['created_at'] ?? date('Y-m-d H:i:s');
            }
            $page_data['updated_at'] = date('Y-m-d H:i:s');
            
            // 重新编码保存
            file_put_contents($existing_file, json_encode($page_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
            $success = '标签页面更新成功';
        }
    }
}

// 处理删除语言包
if (isset($_GET['delete_lang'])) {
    $lang_to_delete = $_GET['delete_lang'];
    
    // 不能删除英文语言包
    if ($lang_to_delete == 'en') {
        $error = '无法删除默认的英文语言包';
    } else {
        $i18n_file = "../i18n/{$domain}/{$lang_to_delete}.json";
        
        if (file_exists($i18n_file)) {
            unlink($i18n_file);
            $success = "语言包 '{$lang_to_delete}' 删除成功";
            
            // 如果当前语言被删除，切换到英文
            if ($current_lang == $lang_to_delete) {
                $current_lang = 'en';
                header("Location: website_pages.php?domain={$domain}&act=tagpage&lang=en&deleted=1");
                exit();
            }
        } else {
            $error = "语言文件未找到";
        }
    }
}

// 处理删除页面
if (isset($_GET['delete'])) {
    $slug = $_GET['delete'];
    $page_dir = "{$pages_dir}/{$slug}";
    
    if (is_dir($page_dir)) {
        deleteDirectory($page_dir);
        $success = '标签页面删除成功';
        $tag_pages = array_diff($tag_pages, [$slug]);
    }
}

function deleteDirectory($dir) {
    if (!is_dir($dir)) return;
    $files = array_diff(scandir($dir), ['.', '..']);
    foreach ($files as $file) {
        $path = $dir . '/' . $file;
        is_dir($path) ? deleteDirectory($path) : unlink($path);
    }
    rmdir($dir);
}

// 获取编辑页面的数据
$edit_slug = $_GET['edit'] ?? '';
$json_content = '';
if ($edit_slug && in_array($edit_slug, $tag_pages)) {
    $lang_file = "{$pages_dir}/{$edit_slug}/{$current_lang}.json";
    if (file_exists($lang_file)) {
        $json_content = file_get_contents($lang_file);
    }
} elseif (!$edit_slug && !isset($_POST['create_page'])) {
    // 为新页面生成默认内容
    $default_content = extractDefaultContent($domain);
    $json_content = json_encode($default_content, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
}

// 获取语言列表
$languages = [];
if ($edit_slug) {
    // 编辑模式：从标签页面目录获取语言列表
    $page_lang_dir = "{$pages_dir}/{$edit_slug}";
    if (is_dir($page_lang_dir)) {
        $files = scandir($page_lang_dir);
        foreach ($files as $file) {
            if (preg_match('/^([a-z]{2})\.json$/', $file, $matches)) {
                $languages[] = $matches[1];
            }
        }
    }
} else {
    // 新建模式：只显示英语
    $languages = ['en'];
}
?>

<div class="row">
    <div class="col-md-8">
        <div class="form-section">
            <?php if ($edit_slug): ?>
            <h4>编辑标签页面: <?php echo htmlspecialchars($edit_slug); ?> - JSON编辑器</h4>
            <?php else: ?>
            <h4>创建新标签页面 - JSON编辑器</h4>
            <?php endif; ?>
            
            <?php if ($error): ?>
            <div class="alert alert-danger">
                <i class="fas fa-exclamation-circle"></i> <?php echo htmlspecialchars($error); ?>
            </div>
            <?php endif; ?>
            
            <?php if ($success): ?>
            <div class="alert alert-success">
                <i class="fas fa-check-circle"></i> <?php echo htmlspecialchars($success); ?>
            </div>
            <?php endif; ?>
            
            <div class="mb-3">
                <div class="row">
                    <div class="col-md-6">
                        <label class="form-label"><i class="fas fa-language"></i> 语言管理</label>
                        <div class="input-group">
                            <select class="form-select" id="languageSelector" onchange="changeLanguage(this.value)">
                                <?php 
                                $language_names = getSupportedLanguages();
                                foreach ($languages as $lang): 
                                    $lang_name = $language_names[$lang] ?? strtoupper($lang);
                                ?>
                                <option value="<?php echo $lang; ?>" <?php echo $current_lang == $lang ? 'selected' : ''; ?>>
                                    <?php echo $lang_name; ?> (<?php echo strtoupper($lang); ?>)
                                </option>
                                <?php endforeach; ?>
                            </select>
                            <?php if ($edit_slug): ?>
                            <button class="btn btn-outline-success" type="button" onclick="addLanguage()">
                                <i class="fas fa-plus"></i> 添加
                            </button>
                            <?php if ($current_lang != 'en'): ?>
                            <button class="btn btn-outline-danger" type="button" onclick="deleteLanguage('<?php echo $current_lang; ?>')">
                                <i class="fas fa-trash"></i> 删除
                            </button>
                            <?php endif; ?>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label"><i class="fas fa-info-circle"></i> 语言统计</label>
                        <div class="p-2 bg-light rounded">
                            <small>
                                <?php if ($edit_slug): ?>
                                当前标签页支持 <strong><?php echo count($languages); ?></strong> 种语言
                                <?php else: ?>
                                新建页面，创建后可添加多语言版本
                                <?php endif; ?>
                            </small>
                        </div>
                        <?php if ($current_lang == 'en' && $edit_slug): ?>
                        <div class="mt-2">
                            <button type="button" class="btn btn-warning btn-sm" onclick="showTranslateModal()">
                                <i class="fas fa-language"></i> 一键翻译
                            </button>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
                <div class="form-text mt-2">
                    <i class="fas fa-info-circle"></i> 
                    <?php if ($edit_slug): ?>
                    英文 (EN) 是默认语言，不能删除。从下拉菜单选择语言进行编辑。
                    <?php else: ?>
                    新建页面将创建英文版本，之后可添加其他语言。
                    <?php endif; ?>
                </div>
            </div>
            
            <form method="post" action="website_pages.php?domain=<?php echo $domain; ?>&act=tagpage&lang=<?php echo $current_lang; ?>" onsubmit="return validateJSON()">
                <?php if ($edit_slug): ?>
                <input type="hidden" name="edit_page" value="1">
                <input type="hidden" name="slug" value="<?php echo htmlspecialchars($edit_slug); ?>">
                <?php else: ?>
                <input type="hidden" name="create_page" value="1">
                <?php endif; ?>
                
                <div class="mb-3">
                    <label for="keyword" class="form-label">关键词</label>
                    <div class="input-group">
                        <input type="text" class="form-control" id="keyword" name="keyword" 
                               placeholder="Youtube Videos Download" 
                               <?php if (!$edit_slug): ?>required<?php endif; ?>
                               oninput="updatePageTitle(this.value)"
                               <?php if ($edit_slug): ?>
                               value="<?php 
                                   // 在编辑模式下，从JSON内容中获取page.title作为默认值
                                   $json_data = json_decode($json_content, true);
                                   echo htmlspecialchars($json_data['page']['title'] ?? '');
                               ?>"
                               <?php endif; ?>>
                        <?php if (!$edit_slug): ?>
                        <button type="button" class="btn btn-outline-secondary" onclick="generateUrlFromKeyword()">
                            <i class="fas fa-link"></i> 生成URL
                        </button>
                        <?php else: ?>
                        <button type="button" class="btn btn-outline-info" onclick="updateKeywordFromTitle()">
                            <i class="fas fa-sync"></i> 从标题同步
                        </button>
                        <?php endif; ?>
                    </div>
                    <div class="form-text">
                        <?php if (!$edit_slug): ?>
                        此关键词将用作页面标题（首字母会自动大写）
                        <?php else: ?>
                        修改关键词后可重新生成提示词，点击同步按钮可从JSON的page.title获取最新值
                        <?php endif; ?>
                    </div>
                </div>
                
                <?php if (!$edit_slug): ?>
                <div class="mb-3">
                    <label for="slug" class="form-label">URL地址</label>
                    <input type="text" class="form-control" id="slug" name="slug" 
                           placeholder="download-youtube-videos" required>
                    <div class="form-text">只允许字母、数字和连字符。页面URL将是: /<?php echo htmlspecialchars($current_lang); ?>/[slug]</div>
                </div>
                <?php endif; ?>
                
                <div class="mb-3">
                    <label class="form-label">
                        <i class="fas fa-code"></i> 标签页面内容 - JSON编辑器
                        <span class="text-muted">(File: pages/<?php echo $domain; ?>/<?php echo $edit_slug ?: '[slug]'; ?>/<?php echo $current_lang; ?>.json)</span>
                    </label>
                    <div id="json-editor" style="height: 600px; border: 1px solid #ddd;"><?php echo htmlspecialchars($json_content); ?></div>
                    <textarea name="json_content" id="json_content" style="display: none;"></textarea>
                    <div class="form-text">
                        <strong>模板内容结构:</strong><br>
                        • <code>page.title</code> - 页面标题和H1<br>
                        • <code>page.meta_description</code> - SEO元描述<br>
                        • <code>content.*</code> - 主要内容区块<br>
                        • <code>instruction.*</code> - 使用说明<br>
                        • <code>advantages.*</code> - 功能优势<br>
                        • <code>features.*</code> - 功能描述<br>
                        • <code>custom_content.content</code> - 自定义HTML内容
                    </div>
                </div>
                
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <button type="button" class="btn btn-secondary" onclick="formatJSON()">
                            <i class="fas fa-magic"></i> 格式化JSON
                        </button>
                        <button type="button" class="btn btn-info" onclick="validateJSONOnly()">
                            <i class="fas fa-check"></i> 验证JSON
                        </button>
                        <button type="button" class="btn btn-warning" onclick="resetToDefault()">
                            <i class="fas fa-undo"></i> 重置为默认
                        </button>
                        <button type="button" class="btn btn-outline-info" onclick="showExtractedKeys()" data-bs-toggle="collapse" data-bs-target="#extractedKeysInfo">
                            <i class="fas fa-eye"></i> 显示解析键值
                        </button>
                    </div>
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> <?php echo $edit_slug ? '更新' : '创建'; ?> 页面
                    </button>
                </div>
            </form>
            
            <!-- Extracted Keys Debug Info -->
            <div class="collapse mt-3" id="extractedKeysInfo">
                <div class="card">
                    <div class="card-header">
                        <i class="fas fa-code"></i> 从文件解析的模板键值
                    </div>
                    <div class="card-body">
                        <div id="extractedKeysDisplay">
                            <p class="text-muted">点击"显示解析键值"来显示动态提取的模板键值</p>
                        </div>
                    </div>
                </div>
            </div>
            
            <!-- Prompt 提示词区域 -->
            <div class="mt-4">
                <div class="card">
                    <div class="card-header">
                        <i class="fas fa-robot"></i> SEO文案生成提示词
                    </div>
                    <div class="card-body">
                        <div class="mb-3">
                            <label for="prompt-textarea-tag" class="form-label">
                                <i class="fas fa-magic"></i> 复制此提示词到ChatGPT生成SEO文案
                            </label>
                            <div class="row mb-2">
                                <div class="col-md-6">
                                    <div class="input-group">
                                        <span class="input-group-text"><i class="fas fa-template"></i> 提示词模板</span>
                                        <select id="prompt-template-tag" class="form-select" onchange="onTemplateChangeTag()">
                                            <option value="">-- 请选择模板 --</option>
                                        </select>
                                        <a href="prompt.php" class="btn btn-outline-info" target="_blank">
                                            <i class="fas fa-cog"></i> 管理
                                        </a>
                                    </div>
                                </div>
                            </div>
                            <textarea id="prompt-textarea-tag" class="form-control" rows="15" placeholder="请先填写关键词并选择提示词模板，然后点击生成提示词"></textarea>
                        </div>
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <button type="button" class="btn btn-primary" onclick="generatePromptTag(event)">
                                    <i class="fas fa-sync"></i> 生成提示词
                                </button>
                                <button type="button" class="btn btn-success" onclick="copyPromptTag()">
                                    <i class="fas fa-copy"></i> 复制提示词
                                </button>
                            </div>
                            <small class="text-muted">基于当前页面内容和关键词自动生成</small>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    
    <div class="col-md-4">
        <div class="card">
            <div class="card-header">
                <i class="fas fa-tags"></i> 现有标签页面
            </div>
            <div class="card-body">
                <?php if (empty($tag_pages)): ?>
                <p class="text-muted mb-0">还没有创建标签页面</p>
                <?php else: ?>
                <div class="list-group">
                    <?php foreach ($tag_pages as $page): ?>
                    <div class="list-group-item d-flex justify-content-between align-items-center">
                        <div>
                            <strong><?php echo htmlspecialchars($page); ?></strong>
                            <br>
                            <small class="text-muted">
                                /<?php echo $current_lang; ?>/<?php echo $page; ?>
                                <a href="http://<?php echo $domain; ?>/<?php echo $current_lang; ?>/<?php echo $page; ?>" 
                                   target="_blank" class="ms-2 text-primary" title="查看页面">
                                    <i class="fas fa-external-link-alt"></i>
                                </a>
                            </small>
                        </div>
                        <div class="btn-group btn-group-sm">
                            <a href="website_pages.php?domain=<?php echo $domain; ?>&act=tagpage&edit=<?php echo $page; ?>&lang=<?php echo $current_lang; ?>" 
                               class="btn btn-outline-primary" title="编辑">
                                <i class="fas fa-edit"></i>
                            </a>
                            <button onclick="confirmDelete('删除该标签页面？', function() { 
                                window.location.href = 'website_pages.php?domain=<?php echo $domain; ?>&act=tagpage&delete=<?php echo $page; ?>'; 
                            })" class="btn btn-outline-danger" title="删除">
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    </div>
                    <?php endforeach; ?>
                </div>
                <?php endif; ?>
            </div>
        </div>
        
        <div class="card mt-3">
            <div class="card-header">
                <i class="fas fa-info-circle"></i> JSON结构指南
            </div>
            <div class="card-body">
                <p class="small mb-2"><strong>动态模板解析:</strong></p>
                <ul class="small">
                    <li>自动从模板文件提取内容</li>
                    <li>解析 <code>__('key', 'default')</code> 函数调用</li>
                    <li>检测 <code>$page_data['section']['key']</code> 模式</li>
                    <li>提取占位符、按钮和列表内容</li>
                    <li>适配任何主题或平台类型</li>
                </ul>
                <p class="small mb-2"><strong>解析的模板文件:</strong></p>
                <ul class="small">
                    <li><code>header.php</code> - Meta标签、导航</li>
                    <li><code>page.php</code> - 主要内容结构</li>
                    <li><code>footer.php</code> - 页脚内容</li>
                    <li><code>home.php</code> - 首页元素</li>
                </ul>
                <p class="small mb-2"><strong>内容优先级:</strong></p>
                <ul class="small">
                    <li>1. 现有i18n翻译值</li>
                    <li>2. 模板默认值</li>
                    <li>3. 自动生成的默认值</li>
                </ul>
                <p class="small mb-0"><strong>URL:</strong> <code>/[语言]/[地址]</code></p>
            </div>
        </div>
    </div>
</div>

<!-- ACE Editor -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/ace/1.22.0/ace.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/ace/1.22.0/mode-json.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/ace/1.22.0/theme-monokai.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/ace/1.22.0/ext-language_tools.min.js"></script>

<script>
// Initialize ACE editor
var editor = ace.edit("json-editor");
editor.setTheme("ace/theme/monokai");
editor.session.setMode("ace/mode/json");
editor.setOptions({
    enableBasicAutocompletion: true,
    enableSnippets: true,
    enableLiveAutocompletion: true,
    showLineNumbers: true,
    tabSize: 2,
    useSoftTabs: true,
    wrap: true,
    showPrintMargin: false,
    fontSize: 14
});

// 更新 page.title 函数
function updatePageTitle(keyword) {
    if (!keyword) return;
    
    try {
        // 获取当前编辑器内容
        const content = editor.getValue();
        const jsonData = JSON.parse(content);
        
        // 格式化关键词 - 每个单词首字母大写
        const formattedKeyword = keyword.toLowerCase().split(' ').map(word => 
            word.charAt(0).toUpperCase() + word.slice(1)
        ).join(' ');
        
        // 更新 page.title
        if (jsonData.page && jsonData.page.title !== undefined) {
            jsonData.page.title = formattedKeyword;
            jsonData.page.h1 = formattedKeyword;

            // 更新编辑器内容
            editor.setValue(JSON.stringify(jsonData, null, 2));
            editor.clearSelection();
        }
    } catch (e) {
        // 如果JSON格式有问题，忽略错误
        console.log('JSON parse error:', e);
    }
}

// 从关键词生成URL
function generateUrlFromKeyword() {
    const keywordInput = document.getElementById('keyword');
    const slugInput = document.getElementById('slug');
    
    if (!keywordInput.value) {
        alert('请先输入关键词');
        keywordInput.focus();
        return;
    }
    
    // 转换关键词为URL格式
    // 1. 转为小写
    // 2. 替换空格为连字符
    // 3. 移除特殊字符
    // 4. 替换多个连字符为单个
    // 5. 移除首尾连字符
    let slug = keywordInput.value
        .toLowerCase()
        .replace(/\s+/g, '-')
        .replace(/[^a-z0-9\-]/g, '')
        .replace(/-+/g, '-')
        .replace(/^-|-$/g, '');
    
    // 如果生成的URL为空，使用默认值
    if (!slug) {
        slug = 'page-' + Date.now();
    }
    
    // 设置URL输入框的值
    slugInput.value = slug;
    
    // 可选：显示一个提示
    const slugGroup = slugInput.parentElement;
    const existingAlert = slugGroup.querySelector('.text-success');
    if (existingAlert) {
        existingAlert.remove();
    }
    
    const successMsg = document.createElement('div');
    successMsg.className = 'text-success small mt-1';
    successMsg.innerHTML = '<i class="fas fa-check"></i> URL已生成';
    slugGroup.appendChild(successMsg);
    
    // 3秒后移除提示
    setTimeout(() => {
        successMsg.remove();
    }, 3000);
}

// 从JSON内容的page.title同步关键词
function updateKeywordFromTitle() {
    try {
        const content = editor.getValue();
        const jsonData = JSON.parse(content);
        const keywordInput = document.getElementById('keyword');
        
        if (jsonData.page && jsonData.page.title) {
            keywordInput.value = jsonData.page.title;
            
            // 显示成功提示
            const inputGroup = keywordInput.parentElement;
            const existingAlert = inputGroup.querySelector('.text-success');
            if (existingAlert) {
                existingAlert.remove();
            }
            
            const successMsg = document.createElement('div');
            successMsg.className = 'text-success small mt-1';
            successMsg.innerHTML = '<i class="fas fa-check"></i> 关键词已从标题同步';
            inputGroup.appendChild(successMsg);
            
            // 3秒后移除提示
            setTimeout(() => {
                successMsg.remove();
            }, 3000);
        } else {
            alert('JSON内容中没有找到page.title字段');
        }
    } catch (e) {
        alert('JSON格式错误，无法同步关键词: ' + e.message);
    }
}

// Store default content for reset function
var defaultContent = <?php echo json_encode(json_encode(extractDefaultContent($domain), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE)); ?>;

// Store extracted keys info for debugging
var extractedKeysInfo = <?php 
    $debug_info = [];
    $template_dir = "../tpl/{$domain}";
    $template_files = ['header.php', 'page.php', 'footer.php', 'home.php'];
    
    foreach ($template_files as $file) {
        $file_path = "{$template_dir}/{$file}";
        if (file_exists($file_path)) {
            $content = file_get_contents($file_path);
            
            // Extract __() calls
            preg_match_all("/\b__\(\s*['\"]([^'\"]+)['\"]\s*(?:,\s*['\"]([^'\"]*)['\"])?\s*\)/", $content, $matches, PREG_SET_ORDER);
            
            foreach ($matches as $match) {
                $debug_info[] = [
                    'file' => $file,
                    'key' => $match[1],
                    'default' => $match[2] ?? '',
                    'type' => '__() function call'
                ];
            }
            
            // Extract $page_data patterns
            preg_match_all('/\$page_data\[[\'"]([^\'"]+)[\'"]\](?:\[[\'"]([^\'"]+)[\'"]\])?(?:\[[\'"]([^\'"]+)[\'"]\])?/', $content, $page_matches, PREG_SET_ORDER);
            
            foreach ($page_matches as $match) {
                $key = $match[1];
                if (isset($match[2])) $key .= '.' . $match[2];
                if (isset($match[3])) $key .= '.' . $match[3];
                
                $debug_info[] = [
                    'file' => $file,
                    'key' => $key,
                    'default' => '',
                    'type' => '$page_data array access'
                ];
            }
        }
    }
    
    echo json_encode($debug_info);
?>;

// Validate JSON only (without saving)
function validateJSONOnly() {
    const content = editor.getValue();
    try {
        JSON.parse(content);
        alert('✅ JSON格式有效！');
    } catch (e) {
        alert('❌ JSON错误：' + e.message);
        
        // Try to highlight the error line
        const match = e.message.match(/position (\d+)/);
        if (match) {
            const position = parseInt(match[1]);
            const lines = content.substring(0, position).split('\n');
            const errorLine = lines.length;
            editor.gotoLine(errorLine);
        }
    }
}

// Format JSON
function formatJSON() {
    try {
        const content = editor.getValue();
        const parsed = JSON.parse(content);
        const formatted = JSON.stringify(parsed, null, 2);
        editor.setValue(formatted);
        editor.clearSelection();
    } catch (e) {
        alert('无法格式化：JSON格式无效\n' + e.message);
    }
}

// Reset to default content
function resetToDefault() {
    if (confirm('这将重置所有内容为默认值。您确定吗？')) {
        editor.setValue(JSON.parse(defaultContent));
        editor.clearSelection();
    }
}

// Show extracted keys from template parsing
function showExtractedKeys() {
    const display = document.getElementById('extractedKeysDisplay');
    
    if (extractedKeysInfo.length === 0) {
        display.innerHTML = '<p class="text-muted">从模板文件中未提取到任何键值。</p>';
        return;
    }
    
    // Group by file
    const groupedByFile = {};
    extractedKeysInfo.forEach(item => {
        if (!groupedByFile[item.file]) {
            groupedByFile[item.file] = [];
        }
        groupedByFile[item.file].push(item);
    });
    
    let html = '<div class="table-responsive"><table class="table table-sm table-striped">';
    html += '<thead><tr><th>File</th><th>Key</th><th>Default Value</th><th>Type</th></tr></thead><tbody>';
    
    Object.keys(groupedByFile).forEach(file => {
        groupedByFile[file].forEach(item => {
            html += `<tr>
                <td><code>${item.file}</code></td>
                <td><code>${item.key}</code></td>
                <td class="text-muted small">${item.default || '(no default)'}</td>
                <td><span class="badge badge-secondary">${item.type}</span></td>
            </tr>`;
        });
    });
    
    html += '</tbody></table></div>';
    html += `<p class="small text-muted mt-2">
        <i class="fas fa-info-circle"></i> 
        Found ${extractedKeysInfo.length} template keys across ${Object.keys(groupedByFile).length} files. 
        These keys are used to build the default JSON structure.
    </p>`;
    
    display.innerHTML = html;
}

// Change language
function changeLanguage(lang) {
    <?php if ($edit_slug): ?>
    window.location.href = 'website_pages.php?domain=<?php echo $domain; ?>&act=tagpage&lang=' + lang + '&edit=<?php echo $edit_slug; ?>';
    <?php else: ?>
    window.location.href = 'website_pages.php?domain=<?php echo $domain; ?>&act=tagpage&lang=' + lang;
    <?php endif; ?>
}

// Validate JSON before submitting
function validateJSON() {
    const content = editor.getValue();
    document.getElementById('json_content').value = content;
    
    try {
        const parsed = JSON.parse(content);
        
        // Basic structure validation
        if (!parsed.page || !parsed.page.title) {
            alert('❌ 错误：page.title 是必要的');
            return false;
        }
        
        return true;
    } catch (e) {
        alert('❌ 无法保存：JSON格式无效\n\n错误：' + e.message);
        return false;
    }
}

// Add new language
function addLanguage() {
    // 获取所有支持的语言
    const allLanguages = <?php echo json_encode(getSupportedLanguages()); ?>;
    const existingLanguages = <?php echo json_encode($languages); ?>;
    
    // 过滤出未添加的语言
    const availableLanguages = {};
    for (const [code, name] of Object.entries(allLanguages)) {
        if (!existingLanguages.includes(code)) {
            availableLanguages[code] = name;
        }
    }
    
    if (Object.keys(availableLanguages).length === 0) {
        alert('所有支持的语言都已添加！');
        return;
    }
    
    // 创建选择列表
    let options = '请选择要添加的语言：\n\n';
    const codes = Object.keys(availableLanguages);
    codes.forEach((code, index) => {
        options += `${index + 1}. ${availableLanguages[code]} (${code})\n`;
    });
    
    const choice = prompt(options + '\n请输入序号或语言代码：');
    
    let lang = null;
    if (choice) {
        // 检查是否输入了序号
        const index = parseInt(choice) - 1;
        if (index >= 0 && index < codes.length) {
            lang = codes[index];
        } else if (/^[a-z]{2}$/.test(choice) && availableLanguages[choice]) {
            lang = choice;
        }
    }
    
    if (lang) {
        const currentContent = editor.getValue();
        
        // Navigate to new language with content
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = 'website_pages.php?domain=<?php echo $domain; ?>&act=tagpage&lang=' + lang<?php echo $edit_slug ? " + '&edit=" . $edit_slug . "'" : ''; ?>;
        
        const input = document.createElement('input');
        input.type = 'hidden';
        input.name = 'json_content';
        input.value = currentContent;
        
        form.appendChild(input);
        document.body.appendChild(form);
        form.submit();
    } else if (lang) {
        alert('语言代码无效。请使用两个小写字母 (例如: en, fr, es)');
    }
}

// Delete language pack
function deleteLanguage(lang) {
    if (lang === 'en') {
        alert('❌ 无法删除默认的英文语言包');
        return;
    }
    
    const allLanguages = <?php echo json_encode(getSupportedLanguages()); ?>;
    const langName = allLanguages[lang] || lang.toUpperCase();
    
    if (confirm(`🗑️ 删除 ${langName} (${lang.toUpperCase()}) 语言包？\n\n此操作无法撤销。JSON文件将被永久删除。`)) {
        // Show loading state
        const deleteBtn = event.target.closest('button');
        const originalHtml = deleteBtn.innerHTML;
        deleteBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
        deleteBtn.disabled = true;
        
        // Navigate to delete URL
        window.location.href = `website_pages.php?domain=<?php echo $domain; ?>&act=tagpage&delete_lang=${lang}<?php echo $edit_slug ? "&edit=" . $edit_slug : ""; ?>`;
    }
}

// Add keyboard shortcut for saving (Ctrl+S)
editor.commands.addCommand({
    name: 'save',
    bindKey: {win: 'Ctrl-S', mac: 'Command-S'},
    exec: function(editor) {
        if (validateJSON()) {
            document.querySelector('form').submit();
        }
    }
});

// Show JSON validation status in real-time
let validationTimeout;
editor.on('change', function() {
    clearTimeout(validationTimeout);
    validationTimeout = setTimeout(function() {
        try {
            JSON.parse(editor.getValue());
            editor.session.clearAnnotations();
        } catch (e) {
            const match = e.message.match(/position (\d+)/);
            if (match) {
                const position = parseInt(match[1]);
                const lines = editor.getValue().substring(0, position).split('\n');
                const errorLine = lines.length - 1;
                
                editor.session.setAnnotations([{
                    row: errorLine,
                    column: 0,
                    text: e.message,
                    type: "error"
                }]);
            }
        }
    }, 500);
});

// 显示翻译模态框
function showTranslateModal() {
    const modal = document.getElementById('translateModal');
    if (modal) {
        modal.style.display = 'block';
    }
}

// 关闭模态框
function closeTranslateModal() {
    const modal = document.getElementById('translateModal');
    if (modal) {
        modal.style.display = 'none';
    }
}

// 执行翻译
function doTranslate() {
    const targetLang = document.getElementById('targetLanguage').value;
    if (!targetLang) {
        alert('请选择目标语言');
        return;
    }
    
    // 验证当前JSON内容
    try {
        JSON.parse(editor.getValue());
    } catch (e) {
        alert('当前JSON格式无效，请先修正');
        return;
    }
    
    // 先保存当前内容
    document.getElementById('json_content').value = editor.getValue();
    
    // 显示加载提示
    const btn = event.target;
    const originalText = btn.innerHTML;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 翻译中...';
    btn.disabled = true;
    
    // 跳转到翻译URL
    window.location.href = 'website_pages.php?domain=<?php echo $domain; ?>&act=tagpage&translate=<?php echo $current_lang; ?>&target_lang=' + targetLang + '&slug=<?php echo $edit_slug; ?>';
}

// 执行一键翻译所有语言
function doTranslateAll() {
    // 验证当前JSON内容
    try {
        JSON.parse(editor.getValue());
    } catch (e) {
        alert('当前JSON格式无效，请先修正');
        return;
    }
    
    if (!confirm('确定要将当前内容翻译到所有38种语言吗？这可能需要较长时间（约5-10分钟）。')) {
        return;
    }
    
    // 先保存当前内容
    document.getElementById('json_content').value = editor.getValue();
    
    const btn = event.target;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 准备翻译...';
    btn.disabled = true;
    
    // 跳转到一键翻译URL
    window.location.href = 'website_pages.php?domain=<?php echo $domain; ?>&act=tagpage&translate_all=<?php echo $current_lang; ?>&slug=<?php echo $edit_slug; ?>';
}

// 执行翻译剩余语言
function doTranslateRemaining() {
    // 验证当前JSON内容
    try {
        JSON.parse(editor.getValue());
    } catch (e) {
        alert('当前JSON格式无效，请先修正');
        return;
    }
    
    if (!confirm('确定要翻译剩余的语言吗？系统会自动检测哪些语言还没有翻译文件。')) {
        return;
    }
    
    // 先保存当前内容
    document.getElementById('json_content').value = editor.getValue();
    
    const btn = event.target;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 检测剩余语言...';
    btn.disabled = true;
    
    // 跳转到翻译剩余语言URL
    window.location.href = 'website_pages.php?domain=<?php echo $domain; ?>&act=tagpage&translate_remaining=<?php echo $current_lang; ?>&slug=<?php echo $edit_slug; ?>';
}

// 更新路由配置
function updateRouter() {
    if (!confirm('确定要更新路由配置吗？系统会根据当前的语言文件自动更新router.json中的支持语言列表。')) {
        return;
    }
    
    const btn = event.target;
    const originalText = btn.innerHTML;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 更新中...';
    btn.disabled = true;
    
    // 发送AJAX请求更新路由
    fetch('website_pages.php?domain=<?php echo $domain; ?>&act=tagpage&update_router=1')
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            alert('✅ ' + data.message + '\n\n支持的语言: ' + data.languages.join(', '));
        } else {
            alert('❌ 更新失败: ' + data.error);
        }
        
        btn.innerHTML = originalText;
        btn.disabled = false;
    })
    .catch(error => {
        alert('❌ 请求失败: ' + error);
        btn.innerHTML = originalText;
        btn.disabled = false;
    });
}

// 加载提示词模板 - 标签页版本
async function loadPromptTemplatesTag() {
    try {
        console.log('开始加载标签页提示词模板...');
        const response = await fetch('lib/get_prompts.php');
        
        if (!response.ok) {
            throw new Error(`HTTP ${response.status}: ${response.statusText}`);
        }
        
        const prompts = await response.json();
        console.log('加载到的提示词模板:', prompts);
        
        const select = document.getElementById('prompt-template-tag');
        if (!select) {
            return []; // 如果选择器不存在（非编辑模式），直接返回
        }
        
        select.innerHTML = '<option value="">-- 请选择模板 --</option>';
        
        if (!Array.isArray(prompts) || prompts.length === 0) {
            console.warn('没有找到提示词模板');
            return [];
        }
        
        prompts.forEach(prompt => {
            const option = document.createElement('option');
            option.value = prompt.id;
            option.textContent = prompt.name;
            option.setAttribute('data-template', prompt.template);
            
            if (prompt.is_active) {
                option.selected = true;
            }
            
            select.appendChild(option);
        });
        
        console.log('标签页提示词模板加载完成，总数:', prompts.length);
        return prompts;
    } catch (error) {
        console.error('加载标签页提示词模板失败:', error);
        
        const select = document.getElementById('prompt-template-tag');
        if (select) {
            select.innerHTML = '<option value="">-- 加载失败，请刷新页面重试 --</option>';
        }
        
        return [];
    }
}

// 模板切换处理 - 标签页版本
function onTemplateChangeTag() {
    const select = document.getElementById('prompt-template-tag');
    const promptTextarea = document.getElementById('prompt-textarea-tag');
    
    if (!select || !promptTextarea) return;
    
    if (select.value) {
        promptTextarea.placeholder = '已选择模板，点击"生成提示词"按钮生成';
    } else {
        promptTextarea.placeholder = '请先选择提示词模板，系统会自动获取关键词并生成提示词';
        promptTextarea.value = '';
    }
}

// 生成提示词 - 标签页版本
async function generatePromptTag(event) {
    try {
        // 检查是否选择了模板
        const templateSelect = document.getElementById('prompt-template-tag');
        const selectedTemplateId = templateSelect.value;
        
        if (!selectedTemplateId) {
            alert('请先选择一个提示词模板');
            return;
        }
        
        // 获取关键词 - 从关键词输入框获取
        const keywordInput = document.getElementById('keyword');
        let keyword = '';
        
        if (keywordInput && keywordInput.value.trim()) {
            keyword = keywordInput.value.trim();
        }
        
        if (!keyword) {
            alert('请先填写关键词');
            if (keywordInput) {
                keywordInput.focus();
            }
            return;
        }
        
        const content = editor.getValue();
        const jsonData = JSON.parse(content);
        
        // 获取 site_name
        const siteName = jsonData.site_name || '站点名称';
        const currentLang = '<?php echo $current_lang; ?>';
        
        // 显示加载状态
        const promptTextarea = document.getElementById('prompt-textarea-tag');
        promptTextarea.value = '正在生成提示词，请稍候...';
        
        // 如果是手动点击，显示加载按钮状态
        let btn = null;
        if (event && event.target) {
            btn = event.target.closest('button');
            if (btn) {
                btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 生成中...';
                btn.disabled = true;
            }
        }
        
        // 调用 SERP API 获取搜索数据
        let searchDataText = '';
        try {
            const response = await fetch(`lib/google_serp_api.php?ajax_search=1&keywords=${encodeURIComponent(keyword)}&lang=${currentLang}&nums=20`);
            const searchData = await response.json();
            
            if (!searchData.error) {
                searchDataText = JSON.stringify(searchData, null, 2);
            } else {
                searchDataText = '（未能获取搜索结果数据）';
            }
        } catch (error) {
            console.error('SERP API调用失败:', error);
            searchDataText = '（搜索数据获取失败）';
        }
        
        // 获取选中的模板
        const selectedOption = templateSelect.options[templateSelect.selectedIndex];
        let promptTemplate = selectedOption.getAttribute('data-template');
        
        if (!promptTemplate) {
            alert('选择的模板无效');
            return;
        }
        
        // 替换变量
        promptTemplate = promptTemplate
            .replace(/\{site_name\}/g, siteName)
            .replace(/\{main_keywords\}/g, keyword)
            .replace(/\{lang_json\}/g, content)
            .replace(/\{serp_json\}/g, searchDataText);

        // 更新 textarea 内容
        promptTextarea.value = promptTemplate;
        
        // 显示成功提示（仅在手动点击时）
        if (btn) {
            btn.innerHTML = '<i class="fas fa-check"></i> 已生成';
            btn.classList.remove('btn-primary');
            btn.classList.add('btn-success');
            btn.disabled = false;
            
            setTimeout(() => {
                btn.innerHTML = '<i class="fas fa-sync"></i> 生成提示词';
                btn.classList.remove('btn-success');
                btn.classList.add('btn-primary');
            }, 2000);
        }
        
    } catch (e) {
        alert('❌ JSON格式错误，无法生成提示词: ' + e.message);
        
        // 恢复按钮状态
        if (btn) {
            btn.innerHTML = '<i class="fas fa-sync"></i> 生成提示词';
            btn.disabled = false;
            btn.classList.remove('btn-success');
            btn.classList.add('btn-primary');
        }
    } finally {
        // 确保按钮始终恢复
        if (btn && btn.disabled) {
            setTimeout(() => {
                if (btn.disabled) {
                    btn.innerHTML = '<i class="fas fa-sync"></i> 生成提示词';
                    btn.disabled = false;
                    btn.classList.remove('btn-success');
                    btn.classList.add('btn-primary');
                }
            }, 3000);
        }
    }
}

// 复制提示词到剪贴板 - 标签页版本
function copyPromptTag() {
    const textarea = document.getElementById('prompt-textarea-tag');
    const content = textarea.value;
    
    if (!content.trim()) {
        alert('请先生成提示词');
        return;
    }
    
    // 复制到剪贴板
    navigator.clipboard.writeText(content).then(function() {
        // 显示成功提示
        const btn = event.target;
        const originalText = btn.innerHTML;
        btn.innerHTML = '<i class="fas fa-check"></i> 已复制';
        btn.classList.remove('btn-success');
        btn.classList.add('btn-info');
        
        setTimeout(() => {
            btn.innerHTML = originalText;
            btn.classList.remove('btn-info');
            btn.classList.add('btn-success');
        }, 2000);
    }).catch(function(err) {
        // 降级方案：选择文本
        textarea.select();
        textarea.setSelectionRange(0, 99999);
        try {
            document.execCommand('copy');
            alert('✅ 提示词已复制到剪贴板');
        } catch (err) {
            alert('❌ 复制失败，请手动选择并复制文本');
        }
    });
}

// 点击模态框外部关闭
window.onclick = function(event) {
    const modal = document.getElementById('translateModal');
    if (event.target == modal) {
        modal.style.display = 'none';
    }
}

// 页面加载完成后初始化提示词模板
document.addEventListener('DOMContentLoaded', function() {
    // 加载提示词模板（创建和编辑模式都需要）
    loadPromptTemplatesTag();
});
</script>

<style>
.ace_editor {
    font-family: 'Monaco', 'Menlo', 'Ubuntu Mono', 'Consolas', 'source-code-pro', monospace;
}
</style>

<!-- 翻译模态框 -->
<?php if ($current_lang == 'en' && $edit_slug): ?>
<div id="translateModal" style="display:none; position:fixed; z-index:1000; left:0; top:0; width:100%; height:100%; background-color:rgba(0,0,0,0.4);">
    <div style="background-color:#fefefe; margin:5% auto; padding:20px; border:1px solid #888; width:80%; max-width:500px; border-radius:5px; max-height:85vh; overflow-y:auto;">
        <span style="color:#aaa; float:right; font-size:28px; font-weight:bold; cursor:pointer;" onclick="closeTranslateModal()">&times;</span>
        <h4><i class="fas fa-language"></i> 一键翻译</h4>
        <hr>
        <p>将当前英文内容翻译为其他语言</p>
        
        <div class="mb-3">
            <label for="targetLanguage" class="form-label">目标语言</label>
            <select class="form-select" id="targetLanguage">
                <option value="">-- 请选择 --</option>
                <?php 
                $supportedLanguages = getSupportedLanguages();
                foreach ($supportedLanguages as $code => $name): 
                    if ($code != 'en' && $code != $current_lang):
                ?>
                <option value="<?php echo $code; ?>"><?php echo $name; ?> (<?php echo strtoupper($code); ?>)</option>
                <?php 
                    endif;
                endforeach; 
                ?>
            </select>
        </div>
        
        <div class="alert alert-info">
            <i class="fas fa-info-circle"></i> 注意：
            <ul class="mb-0 mt-2">
                <li>翻译基于 ChatGPT 3.5 API</li>
                <li>大文件会自动分批翻译</li>
                <li>占位符如 {site_name} 会被保留</li>
                <li>翻译可能需要一些时间，请耐心等待</li>
            </ul>
        </div>
        
        <div class="alert alert-warning">
            <i class="fas fa-magic"></i> <strong>批量翻译选项</strong><br>
            <small>
                • <strong>翻译全部语言</strong>: 翻译所有38种语言（会覆盖已存在的翻译）<br>
                • <strong>翻译剩余语言</strong>: 只翻译那些还没有翻译文件的语言
            </small>
        </div>
        
        <div class="d-flex justify-content-between align-items-center">
            <div class="d-flex gap-2">
                <button type="button" class="btn btn-success" onclick="doTranslateAll()">
                    <i class="fas fa-globe"></i> 翻译全部语言
                </button>
                <button type="button" class="btn btn-info" onclick="doTranslateRemaining()">
                    <i class="fas fa-plus-circle"></i> 翻译剩余语言
                </button>
            </div>
            <div class="d-flex gap-2">
                <button type="button" class="btn btn-secondary" onclick="closeTranslateModal()">取消</button>
                <button type="button" class="btn btn-primary" onclick="doTranslate()">
                    <i class="fas fa-language"></i> 翻译单种语言
                </button>
            </div>
        </div>
    </div>
</div>
<?php endif; ?>