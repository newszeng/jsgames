...
function createWebsiteFiles($domain, $type, $theme, $site_name) {
    ...
    $router_config = str_replace('{SITE_NAME}', ucfirst($type) . ' Downloader', $router_template);
    ...
    foreach ($static_dirs as $dir) {
        @mkdir("../static/{$domain}/{$dir}", 0777, true);
    }
    ...
    if (is_dir($theme_dir)) {
        ...
        foreach ($dirs as $dir) {
            if ($dir != '.' && $dir != '..' && is_dir($theme_dir . '/' . $dir)) {
                ...
                $description = '<p class="text-muted">暂无描述信息</p>';
                if (file_exists($readme_file)) {
                    $markdown = file_get_contents($readme_file);
                    $description = parseMarkdown($markdown);
                }
                $themes[] = [
                    'name' => $dir,
...
<div class="row">
    <div class="col-md-8">
        <div class="form-section">
            <h4><?php echo $edit_mode ? '编辑' : '添加新'; ?>网站</h4>
            ...
            <form method="post">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="site_name" name="site_name" 
                           ...
                           placeholder="My YouTube Downloader" required>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="type" name="type" required onchange="loadThemes(this.value)">
                        ...
                        <?php foreach ($supportedPlatforms as $key => $name): ?>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <ul class="small">
                    ...
                    <li>创建路由配置</li>
                    <li>创建静态资源目录</li>
                    <li>网站将准备就绪，可进行内容管理</li>
                </ul>
                
                <hr>
                
                <h6>可用主题</h6>
                <p class="small">每个平台都有不同的主题，具有独特的设计和功能。选择平台以查看可用主题。</p>
            </div>
        </div>
    </div>
</div>
